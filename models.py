...
class NSDSlotTaggingModel(Model):
    ...
    def __init__(self, 
        ...
        if bert_embedder:
            ...
            # print("####### basic_embedder = ",self.basic_embedder)
    ...
    def forward(self,
        ...
        if self.use_bert:
            ...
            output["embeddings"] = embeddings #embeddings shape: torch.Size([64, 23, 1024])
        ...
        if self.encoder:
            ...
            output["encoder_out"] = encoder_out # encoder out shape: torch.Size([batchsize, 23, 256])

        else:
            encoder_out = embeddings
        
        tag_logits = self.hidden2tag(encoder_out)
        output["tag_logits"] = tag_logits # tag logits shape: torch.Size([batchsize, 23, 12])
        ...
        if self.use_crf:
            ...
            sequence_length = torch.sum(mask,-1)
            predicted_tags = [predicted_tags[i,:seq_len].tolist() for i,seq_len in enumerate(sequence_length)]
            output["predicted_tags"] = predicted_tags   # pylint: disable=no-member  # torch.Size([batchsize, 23])
